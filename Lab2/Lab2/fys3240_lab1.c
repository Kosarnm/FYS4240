 ...
 *	@param value the 4-bit value to be written to the led
 */
void put_on_leds(uint8_t value)
{
	/* Your code here! */
	uint8_t temp  = (value << 4 ) & LED_PORT_MASK;
	
	write_port_value(&LED_PORT, (~value)<<4, LED_PORT_MASK);
	/*write_port_value(&PORTB, temp, 0xF0);*/
	
}

/* write value to the led
 *	@return the 4-bit value read from the buttons
 */
uint8_t getButtonInput(void)
{
	...
	return read_port_value(&BUTTON_PORT) & BUTTON_PORT_MASK;
}
